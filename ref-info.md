...
- **注1：在内核实现赛道，并不要求参赛队要从零开发os kernel，可以参考和复制如下的内核参考例子（即作为base OS）或内核模块参考，进一步实现和改进可通过测试用例的内核功能。重点是参赛队自己独立完成的内核增量工作。**
...
- **注4：如果参赛队把参考和复制的内核模块等做了改进后，鼓励通过github的PR（提交git patch）等方式回馈给原作者和原项目。**
